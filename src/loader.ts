...
import { mergeProps } from './customize/icwProps';
  ...
  const parcelConfigGetter: ParcelConfigObjectGetter = (remountContainer = initialContainer) => {
    ...
    const parcelConfig: ParcelConfigObject = {
      ...
      mount: [
        ...
        async (props) =>
          mount(
            mergeProps({ ...props, container: appWrapperGetter(), setGlobalState, onGlobalStateChange }, appInstanceId),
          ),
        ...
        async (props) => {
          updateFn = () => {
            return update(
              mergeProps(
                { ...props, container: appWrapperGetter(), setGlobalState, onGlobalStateChange },
                appInstanceId,
              ),
            );
          };
          /// 解决当前应用没有变更.
          /// 但是需要将query重新assign到props的情况.
          window.addEventListener('single-spa:before-no-app-change', updateFn);
        },
      ],
      unmount: [
        ...
        async () => {
          window.removeEventListener('single-spa:before-no-app-change', updateFn);
        },
      ],
    };

    if (typeof update === 'function') {
      parcelConfig.update = (props) => {
        return update(mergeProps({ ...props, setGlobalState, onGlobalStateChange }, appInstanceId));
      };
    }
